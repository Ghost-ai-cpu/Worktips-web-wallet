# Worktips Web Wallet - a lightweight web wallet for Worktips based on Worktips 

In this example [restbed](https://github.com/Corvusoft/restbed/) is used to
demonstrate how to provide Worktips related JSON REST service. 

Worktips Web Wallet is an open source implementation of backend of
https://myworktips.com/ adapted for Worktips. The frontend, which includes HTML, CSS, JavaScript, was adapted
from (and originally developed by) https://myworktips.com/.

Worktips wallet backend is open sourced, free
...
The Worktips Web Wallet consists of four components that need to be setup for it to work:
 ...
 from the Worktips Web Wallet into the Worktips Network.
...
Worktips Web Wallet is not as fast as MyWorktips.
...
Below are example and basic instructions on how to setup up and run Worktips Web Wallet on Ubuntu 16.04.
...
```bash
# first install Worktips dependencies
sudo apt update
...
git clone --recursive https://github.com/worktips-coin/worktips

cd worktips/

# checkout last worktips version -- v2.0.0-rc as of this edit, replace "release_tag" below with the current release tag
git checkout -b last_release release_tag

USE_SINGLE_BUILDDIR=1 make
```

#### Compilation of the Worktips Web Wallet (don't run it yet)

Once Worktips was downloaded and compiled, we can download Worktips Web Wallet and compile it.
...
git clone https://github.com/worktips-coin/worktips-web-wallet.git
...
Create mariadb container called `worktipslockerdb` and root password of `root` (change these  how you want).

```
docker run --name worktipslockerdb -p 3306:3306 -e MYSQL_ROOT_PASSWORD=root -d mariadb
```

Create worktipslocker database called `worktipslocker`.

```
cd worktips-web-wallet/sql
docker exec -i worktipslockerdb mysql -uroot -proot < db.sql
```

#### PhpMyAdmin (using docker)
A good way to manage/view the worktipslocker database is through the
[PhpMyAdmin in docker](https://hub.docker.com/r/phpmyadmin/phpmyadmin/). Using docker,
this can be done:

```
docker run --name myadmin -d --link worktipslockerdb:db -p 8080:80 phpmyadmin/phpmyadmin
```

where `worktipslockerdb` is the name of docker container with mariadb, set in previous step.
...
```
docker run --name html -p 80:80 -v /home/username/worktipslocker/html:/usr/share/nginx/html:ro -d nginx
```

where `worktipshtml` is docker container name, `80:80` will expose the frontend
on port 80 of the localhost, and `/home/username/worktipslocker/html` is the location on your host computer where the
...
#### Run Worktips Web Wallet

Command line options

```bash
./worktips-web-wallet -h
...
Before running `worktips-web-wallet`:
...
```bash
./worktips-web-wallet -t
```

To start for stagenet:
```bash
./worktips-web-wallet -s
```

To start for stagenet with non-default location of `config.json` file:

```bash
./worktips-web-wallet -s -c /path/to/config.json
```


## Worktips Web Wallet JSON REST API

Example JSON REST requests and their responses of [Worktips Web Wallet](https://github.com/worktips-coin/worktips-web-wallet) are provided below. The long term goal
...
the Worktips Web Wallet API does not fully conform to that.

#### get_version

Get version of the Worktips Web Wallet, its API and Worktips.
...
```json
{
  ...
  "worktips_version_full": "0.2.0-master"
}
```
...
Login an existing or new user into Worktips Web Wallet
...
Submit raw tx in hex generated by the frontend to be relayed to the Worktips Network.
