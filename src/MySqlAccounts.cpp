...
bool
MysqlInputs::select_for_out(const uint64_t& output_id, vector<WtipInput>& ins)
{
    try
    {
        conn->check_if_connected();

        Query query = conn->query(WtipInput::SELECT_STMT4);
        query.parse();
...
bool
MysqlOutpus::exist(const string& output_public_key_str, WtipOutput& out)
{
    try
    {
        conn->check_if_connected();

        Query query = conn->query(WtipOutput::EXIST_STMT);
        query.parse();
...
MysqlTransactions::mark_spendable(const uint64_t& tx_id_no, bool spendable)
{
    try
    {
        ...
        Query query = conn->query(spendable ?
                                  WtipTransaction::MARK_AS_SPENDABLE_STMT
                                            : WtipTransaction::MARK_AS_NONSPENDABLE_STMT);
        query.parse();
...
MysqlTransactions::delete_tx(const uint64_t& tx_id_no)
{
    try
    {
        ...
        Query query = conn->query(WtipTransaction::DELETE_STMT);
        query.parse();
...
bool
MysqlTransactions::exist(const uint64_t& account_id, const string& tx_hash_str, WtipTransaction& tx)
{
    try
    {
        conn->check_if_connected();

        Query query = conn->query(WtipTransaction::EXIST_STMT);
        query.parse();

        vector<WtipTransaction> outs;
...
MysqlTransactions::get_total_recieved(const uint64_t& account_id, uint64_t& amount)
{
    try
    {
        ...
        Query query = conn->query(WtipTransaction::SUM_WTIP_RECIEVED);
        query.parse();
...
bool
MysqlPayments::select_by_payment_id(const string& payment_id, vector<WtipPayment>& payments)
{
    ...
    try
    {
        ...
        Query query = conn->query(WtipPayment::SELECT_STMT2);
        query.parse();
...
bool
MySqlAccounts::select(const string& address, WtipAccount& account)
{
    try
    {
        conn->check_if_connected();

        Query query = conn->query(WtipAccount::SELECT_STMT2);
        query.parse();

        vector<WtipAccount> res;
...
template
uint64_t MySqlAccounts::insert<WtipAccount>(const WtipAccount& data_to_insert);
template
uint64_t MySqlAccounts::insert<WtipTransaction>(const WtipTransaction& data_to_insert);
template
uint64_t MySqlAccounts::insert<WtipOutput>(const WtipOutput& data_to_insert);
template
uint64_t MySqlAccounts::insert<WtipInput>(const WtipInput& data_to_insert);
template
uint64_t MySqlAccounts::insert<WtipPayment>(const WtipPayment& data_to_insert);
...
// Explicitly instantiate insert template for our tables
template
uint64_t MySqlAccounts::insert<WtipOutput>(const vector<WtipOutput>& data_to_insert);
template
uint64_t MySqlAccounts::insert<WtipInput>(const vector<WtipInput>& data_to_insert);
...
template
bool MySqlAccounts::select<WtipAccount>(uint64_t account_id, vector<WtipAccount>& selected_data);

template
bool MySqlAccounts::select<WtipTransaction>(uint64_t account_id, vector<WtipTransaction>& selected_data);

template
bool MySqlAccounts::select<WtipOutput>(uint64_t account_id, vector<WtipOutput>& selected_data);

template // this will use SELECT_STMT2 which selectes based on transaction id, not account_id,
bool MySqlAccounts::select<WtipOutput, 2>(uint64_t tx_id, vector<WtipOutput>& selected_data);

template
bool MySqlAccounts::select<WtipInput>(uint64_t account_id, vector<WtipInput>& selected_data);

template
bool MySqlAccounts::select<WtipPayment>(uint64_t account_id, vector<WtipPayment>& selected_data);

template // this will use SELECT_STMT2 which selectes based on transaction id, not account_id,
bool MySqlAccounts::select<WtipInput, 2>(uint64_t tx_id, vector<WtipInput>& selected_data);
...
template
bool MySqlAccounts::update<WtipAccount>(WtipAccount const& orginal_row, WtipAccount const& new_row);

template
bool MySqlAccounts::update<WtipPayment>(WtipPayment const& orginal_row, WtipPayment const& new_row);
...
template // this will use SELECT_STMT2 which selectes based on transaction id, not account_id,
bool MySqlAccounts::select_for_tx<WtipOutput>(uint64_t tx_id, vector<WtipOutput>& selected_data);


template // this will use SELECT_STMT2 which selectes based on transaction id, not account_id,
bool MySqlAccounts::select_for_tx<WtipInput>(uint64_t tx_id, vector<WtipInput>& selected_data);
...
//template
//bool MySqlAccounts::select_by_primary_id<WtipTransaction>(uint64_t id, WtipTransaction& selected_data);

template
bool MySqlAccounts::select_by_primary_id<WtipInput>(uint64_t id, WtipInput& selected_data);

template
bool MySqlAccounts::select_by_primary_id<WtipOutput>(uint64_t id, WtipOutput& selected_data);

template
bool MySqlAccounts::select_by_primary_id<WtipPayment>(uint64_t id, WtipPayment& selected_data);

bool
MySqlAccounts::select_txs_for_account_spendability_check(
        const uint64_t& account_id, vector<WtipTransaction>& txs)
{
    ...
    for (auto it = txs.begin(); it != txs.end(); )
    {
        ...
        if (bool {tx.spendable} == false)
        {
            ...
            if (current_bc_status->is_tx_unlocked(tx.unlock_time, tx.height))
            {
                ...
                if (no_row_updated != 1)
                {
                    cerr << "no_row_updated != 1 due to  wtip_accounts->mark_tx_spendable(tx.id)\n";
                    return false;
                }
            ...
            else
            {
                ...
                if (blockchain_tx_id != tx.blockchain_tx_id)
                {
                    ...
                    if (no_row_updated != 1)
                    {
                        cerr << "no_row_updated != 1 due to  wtip_accounts->delete_tx(tx.id)\n";
                        return false;
                    }
...
bool
MySqlAccounts::select_inputs_for_out(const uint64_t& output_id, vector<WtipInput>& ins)
{
    return mysql_in->select_for_out(output_id, ins);
}

bool
MySqlAccounts::output_exists(const string& output_public_key_str, WtipOutput& out)
{
    return mysql_out->exist(output_public_key_str, out);
}

bool
MySqlAccounts::tx_exists(const uint64_t& account_id, const string& tx_hash_str, WtipTransaction& tx)
{
...
bool
MySqlAccounts::select_payment_by_id(const string& payment_id, vector<WtipPayment>& payments)
{
