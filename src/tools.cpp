...
string
wtip_amount_to_str(const uint64_t& wtip_amount, string format)
{
    return fmt::format(format, WTIP_AMOUNT(wtip_amount));
}
...
{
    if (!wtipeg::parse_str_address(addres_str, address, net_type))
        return false;

    if (!wtipeg::parse_str_secret_key(viewkey_str, viewkey))
          return false;
