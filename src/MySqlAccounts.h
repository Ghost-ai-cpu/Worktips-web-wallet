...
#ifndef RESTBED_WTIP_MYSQLACCOUNTS_H
#define RESTBED_WTIP_MYSQLACCOUNTS_H
...
class WtipTransactionWithOutsAndIns;
class WtipInput;
class WtipOutput;
class WtipTransaction;
class WtipPayment;
class WtipAccount;
class TxSearch;
class Table;
...
public:
    ...
    bool
    select_for_out(const uint64_t& output_id, vector<WtipInput>& ins);
};
...
public:
    ...
    bool
    exist(const string& output_public_key_str, WtipOutput& out);
};
...
public:
    ...
    bool
    exist(const uint64_t& account_id, const string& tx_hash_str, WtipTransaction& tx);
...
public:
    ...
    bool
    select_by_payment_id(const string& payment_id, vector<WtipPayment>& payments);
};
...
public:
    ...
    bool
    select(const string& address, WtipAccount& account);
    ...
    bool
    select_inputs_for_out(const uint64_t& output_id, vector<WtipInput>& ins);

    bool
    output_exists(const string& output_public_key_str, WtipOutput& out);

    bool
    tx_exists(const uint64_t& account_id, const string& tx_hash_str, WtipTransaction& tx);
    ...
    bool
    select_payment_by_id(const string& payment_id, vector<WtipPayment>& payments);

    bool
    update_payment(WtipPayment& payment_orginal, WtipPayment& payment_new);
...
#endif //RESTBED_WTIP_MYSQLACCOUNTS_H
