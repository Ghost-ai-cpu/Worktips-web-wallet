...
#define WORKTIPS_DEFAULT_LOG_CATEGORY "worktips"
        ...
        string& tx_hash_with_payment)
{
    ...
    for (transaction& tx: txs_to_check)
    {
        ...
        std::vector<public_key> tx_pub_keys = wtipeg::get_tx_pub_keys_from_received_outs(tx);

        public_key tx_pub_key;

        // wtip puts 2 keys sometimes in normal transactions.  previously the above
        ...
        for (auto& out: outputs)
        {
            ...
            if (mine_output && tx.version >= txversion::v2_ringct)
            {
...
CurrentBlockchainStatus::start_tx_search_thread(
        WtipAccount acc, std::unique_ptr<TxSearch> tx_search)
{
    ...
    try
    {
        // launch SearchTx thread for the given wtip account
...
bool
CurrentBlockchainStatus::get_wtip_address_viewkey(
        ...
        secret_key& viewkey)
{
    ...
    address = get_search_thread(address_str).get_wtip_address_viewkey().first;
    viewkey = get_search_thread(address_str).get_wtip_address_viewkey().second;
