...
#include "YourWorktipsRequests.h"
...
YourWorktipsRequests::YourWorktipsRequests(
        shared_ptr<MySqlAccounts> _acc, 
        shared_ptr<CurrentBlockchainStatus> _current_bc_status):
    wtip_accounts {_acc}, current_bc_status {_current_bc_status}
{
...
void
YourWorktipsRequests::login(const shared_ptr<Session> session, const Bytes & body)
{
    json j_response;
    json j_request;
    ...
    try
    {
        wtip_address = j_request["address"];
    ...
    if (!wtip_accounts->select(wtip_address, acc))
    {
        ...
        DateTime blk_timestamp_mysql_format
                = WtipTransaction::timestamp_to_DateTime(
                    current_blockchain_timestamp);


        // create new account
        WtipAccount new_account(
                       mysqlpp::null,
                       wtip_address,
                       make_hash(view_key),
        ...
        if ((acc_id = wtip_accounts->insert(new_account)) == 0)
        {
    ...
    if (login_and_start_search_thread(wtip_address, view_key, acc, j_response))
    {
    ...
    } // else  if (login_and_start_search_thread(wtip_address,
...
void
YourWorktipsRequests::get_address_txs(
        const shared_ptr< Session > session, const Bytes & body)
{
    ...
    j_response = json {
            ...
            {"scanned_height"         , 0},    // not used. just to match worktips
...
//        }
        ...
        vector<WtipTransaction> txs;

        wtip_accounts->select(acc.id.data, txs);

        if (wtip_accounts->select_txs_for_account_spendability_check(
                    acc.id.data, txs))
        {
             ...
             */
            for (WtipTransaction const& tx: txs)
            {
                vector<WtipOutput> tx_outs;
                ...
                if (wtip_accounts->select_for_tx(tx.id.data, tx_outs))
                {
					for (WtipOutput const& tx_out : tx_outs)
					{
                ...
                vector<WtipInput> inputs;

                if (wtip_accounts->select_for_tx(tx.id.data, inputs))
                {
                    json j_spent_outputs = json::array();

                    uint64_t total_spent {0};

                    for (WtipInput input: inputs)
                    {
                        WtipOutput out;

                        if (wtip_accounts->select_by_primary_id(
                                    input.output_id, out))
                        {
                ...
                } // if (wtip_accounts->select_inputs_for_tx(tx.id, inputs))
        ...
        } // if (wtip_accounts->select_txs_for_ac
...
void
YourWorktipsRequests::get_address_info(
        const shared_ptr< Session > session, const Bytes & body)
{
    json j_response;
    json j_request;
    ...
    j_response = json {
            {"locked_funds"           , "0"},    // locked wtip (e.g., younger than 10 blocks)
            {"total_received"         , "0"},    // calculated in this function
            {"total_sent"             , "0"},    // calculated in this function
            {"scanned_height"         , 0},    // not used. it is here to match worktips
            {"scanned_block_height"   , 0},    // taken from Accounts table
            {"scanned_block_timestamp", 0},    // taken from Accounts table
            {"start_height"           , 0},    // not used, but available in Accounts table.
                                               // it is here to match worktips
            {"blockchain_height"      , 0},    // current blockchain height
    ...
    if (login_and_start_search_thread(wtip_address, view_key, acc, j_response))
    {
        ...
        current_bc_status->ping_search_thread(wtip_address);

        uint64_t current_searched_blk_no {0};

        if (current_bc_status->get_searched_blk_no(
                    wtip_address, current_searched_blk_no))
        {
            ...
            if (current_searched_blk_no > acc.scanned_block_height + 10)
            {
                current_bc_status->set_new_searched_blk_no(
                            wtip_address, acc.scanned_block_height);
            }
        }
                    ...
                    acc.id.data, txs))
        {
            ...
            for (WtipTransaction tx: txs)
            {
                vector<WtipOutput> outs;

                if (wtip_accounts->select_for_tx(tx.id.data, outs))
                {
                    for (WtipOutput &out: outs)
                    {
                        // check if the output, has been spend
                        vector<WtipInput> ins;

                        if (wtip_accounts->select_inputs_for_out(
                                    out.id.data, ins))
                        {
                            for (WtipInput& in: ins)
                            {
                                j_spent_outputs.push_back({
                ...
                } //  if (wtip_accounts->select_outputs_for_tx(tx.id, outs))
        ...
        } // if (wtip_accounts->select_txs_for_account_spendability_check(acc.id, txs))
...
void
YourWorktipsRequests::get_unspent_outs(
...
//                = current_bc_status->get_current_blockchain_height();
        ...
        if (wtip_accounts->select(acc.id.data, txs))
        {
            // we found some txs.
...
//                }
        ...
        } //  if (wtip_accounts->select_txs(acc.id, txs))
...
void
YourWorktipsRequests::get_random_outs(
        const shared_ptr< Session > session, const Bytes & body)
{
    json j_request;
    json j_response;
...
void
YourWorktipsRequests::submit_raw_tx(
        const shared_ptr< Session > session, const Bytes & body)
{
...
void
YourWorktipsRequests::import_wallet_request(
        const shared_ptr< Session > session, const Bytes & body)
{
    json j_request = body_to_json(body);

    string wtip_address   = j_request["address"];
    ...
    if (current_bc_status->get_bc_setup().import_fee == 0)
    {
        ...
        if (!current_bc_status->set_new_searched_blk_no(wtip_address, 0))
        {
    ...
    if (!wtip_accounts->select(wtip_address, acc))
    {
        cerr << "wtip_address does not exists! " << endl;
    ...
    if (wtip_accounts->select(acc.id.data, wtip_payments))
    {
        ...
        WtipPayment& wtip_payment = wtip_payments[0];

        bool request_fulfilled = bool {wtip_payment.request_fulfilled};

        string integrated_address =
                current_bc_status->get_account_integrated_address_as_str(
                        wtip_payment.payment_id);

        j_response["payment_id"]        = wtip_payment.payment_id;
        j_response["import_fee"]        = std::to_string(wtip_payment.import_fee);
        ...
        if (!request_fulfilled)
        {
            ...
            if(current_bc_status->search_if_payment_made(
                    wtip_payment.payment_id,
                    wtip_payment.import_fee,
                    tx_hash_with_payment))
            {
                WtipPayment updated_wtip_payment = wtip_payment;

                // updated values
                updated_wtip_payment.request_fulfilled = true;
                updated_wtip_payment.tx_hash           = tx_hash_with_payment;

                // save to mysql
                if (wtip_accounts->update(wtip_payment, updated_wtip_payment))
                {
                    ...
                    if (wtip_accounts->select(wtip_address, acc))
                    {
                        WtipAccount updated_acc = acc;

                        updated_acc.scanned_block_height = 0;

                        if (wtip_accounts->update(acc, updated_acc))
                        {
                            ...
                            if (!current_bc_status
                                    ->set_new_searched_blk_no(wtip_address, 0))
                            {
    ...
    } //  if (wtip_accounts->select_payment_by_address(wtip_address, wtip_payment))
    else
    {
        ...
        WtipPayment wtip_payment;

        wtip_payment.id                = mysqlpp::null;
        wtip_payment.account_id        = acc.id.data;
        wtip_payment.payment_id        = pod_to_hex(random_payment_id8);
        wtip_payment.import_fee        = current_bc_status
                                            ->get_bc_setup().import_fee; // wtip
        wtip_payment.request_fulfilled = false;
        wtip_payment.tx_hash           = ""; // no tx_hash yet with the payment
        wtip_payment.payment_address   = integrated_address;

        if ((payment_table_id = wtip_accounts->insert(wtip_payment)) != 0)
        {
            ...
            j_response["new_request"]       = true;
            j_response["request_fulfilled"]
                                         = bool {wtip_payment.request_fulfilled};
            j_response["payment_address"]   = wtip_payment.payment_address;
            j_response["status"]            = "Payment not yet received";
            j_response["error"]             = "";
        }
    }
...
void
YourWorktipsRequests::import_recent_wallet_request(
        const shared_ptr< Session > session, const Bytes & body)
{
    json j_response;
    json j_request;
    ...
    if (wtip_accounts->select(wtip_address, acc))
    {
        ...
        if (updated_acc.scanned_block_height >= no_blocks_to_import)
        {
            ...
            if (wtip_accounts->update(acc, updated_acc))
            {
                // change search blk number in the search thread
                if (!current_bc_status
                        ->set_new_searched_blk_no(wtip_address,
                                    updated_acc.scanned_block_height))
                {
                    cerr << "Updating searched_blk_no failed!" << endl;
                    j_response["Error"]  = "Updating searched_blk_no failed!";
        ...
        const shared_ptr< Session > session, const Bytes & body)
{
    ...
    if (tx_found)
    {
        ...
        j_response["pub_key"] = "";
        std::vector<public_key> tmp_keys = wtipeg::get_tx_pub_keys_from_received_outs(tx);
        if (tmp_keys.size() >= 2)
        {
        ...
        array<uint64_t, 4> const& sum_data = wtipeg::summary_of_in_out_rct(
                tx, output_pub_keys, input_key_imgs);

        wtip_outputs       = sum_data[0];
        wtip_inputs        = sum_data[1];
        mixin_no          = sum_data[2];
        num_nonrct_inputs = sum_data[3];

        j_response["wtip_outputs"]    = wtip_outputs;
        j_response["wtip_inputs"]     = wtip_inputs;
        ...
        // to get info about recived wtip in this tx, we calculate it from
        ...
        if (current_bc_status->get_wtip_address_viewkey(
                    wtip_address, address_info, viewkey))
        {
            ...
            if (wtip_accounts->select(wtip_address, acc))
            {
                ...
                if (!tx_in_mempool)
                {
                    ...
                    WtipTransaction wtip_tx;

                    if (wtip_accounts->tx_exists(
                                acc.id.data, tx_hash_str, wtip_tx))
                    {
                        j_response["payment_id"] = wtip_tx.payment_id;
                        j_response["timestamp"]
                                = static_cast<uint64_t>(wtip_tx.timestamp*1e3);

                        vector<WtipInput> inputs;

                        if (wtip_accounts->select_for_tx(
                                    wtip_tx.id.data, inputs))
                        {
                            json j_spent_outputs = json::array();
                        ...
                        } // if (wtip_accounts->select_inputs_

                    }  // if (wtip_accounts->tx_exists(acc.id
                ...
                else
                {
                    ...
                    if (current_bc_status->get_known_outputs_keys(
                            wtip_address, known_outputs_keys))
                    {
                        ...
                        for (auto& in_info: oi_identification.identified_inputs)
                        {
                            ...
                            if (wtip_accounts->output_exists(out_pub_key, out))
                            {
        ...
        } //  if (current_bc_status->get_wtip_add
...
void
YourWorktipsRequests::get_version(
        const shared_ptr< Session > session,
        const Bytes & body)
{
...
shared_ptr<Resource>
YourWorktipsRequests::make_resource(
        function< void (YourWorktipsRequests&, const shared_ptr< Session >,
...
void
YourWorktipsRequests::generic_options_handler(
        const shared_ptr< Session > session )
{
    const auto request = session->get_request( );
...
YourWorktipsRequests::make_headers(
...
void
YourWorktipsRequests::print_json_log(const string& text, const json& j)
{
    cout << text << '\n' << j.dump(4) << endl;
}
...
string
YourWorktipsRequests::body_to_string(const Bytes & body)
{
    return string(reinterpret_cast<const char *>(body.data()), body.size());
}

json
YourWorktipsRequests::body_to_json(const Bytes & body)
{
...
uint64_t
YourWorktipsRequests::get_current_blockchain_height()
{
    return current_bc_status->get_current_blockchain_height();
}

bool
YourWorktipsRequests::login_and_start_search_thread(
...
void
YourWorktipsRequests::session_close(
...
bool
YourWorktipsRequests::parse_request(
        const Bytes& body,
        ...
        json& j_response)
{
    ...
    {
        cerr << "YourWorktipsRequests::parse_request: " << e.what() << endl;
