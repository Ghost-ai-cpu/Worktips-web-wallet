        ...
        while(continue_search)
        {
            ...
            for (auto const& tx_tuple: txs_data)
            {
                ...
                if (!oi_identification.identified_outputs.empty())
                {
                    ...
                    tx_mysql_id = wtip_accounts->insert(tx_data);
                    ...
                    vector<WtipOutput> outputs_found;
                    ...
                    uint64_t no_rows_inserted
                            = wtip_accounts->insert(outputs_found);
                ...
                if (!oi_identification.identified_inputs.empty())
                {
                    ...
                    for (auto& in_info: oi_identification.identified_inputs)
                    {
                        ...
                        } // if (wtip_accounts->output_exists(o
                    ...
                    if (!inputs_found.empty())
                    {
                        ...
                        for (const WtipInput& in_data: inputs_found)
                        ...
                        for (WtipInput& in_data: inputs_found)
                            in_data.tx_id = tx_mysql_id; // set tx id now.
                                                        //before we made it 0

                        uint64_t no_rows_inserted
                                = wtip_accounts->insert(inputs_found);
...
TxSearch::populate_known_outputs()
{
    ...
    if (wtip_accounts->select(acc->id.data, outs))
    {
        for (const WtipOutput& out: outs)
        {
...
{   
    ...
    *j_transactions = json::array();
    ...
    auto local_wtip_accounts = make_shared<MySqlAccounts>(current_bc_status);
    ...
    for (auto const& mtx: mempool_txs)
    {
        ...
        // FIRSt step. to search for the incoming wtip, we use address,
        // viewkey and
        // outputs public key.
        oi_identification.identify_outputs();
        ...
        if (!oi_identification.identified_inputs.empty())
        {
            ...
            for (auto& in_info: oi_identification.identified_inputs)
            {
                ...
                if (local_wtip_accounts->output_exists(
                            pod_to_hex(in_info.out_pub_key), out))
                {
            ...
            if (!spend_keys.empty())
            {
                ...
                else
                {
                    ...
                    j_tx["tx_pub_key"]     = oi_identification.get_tx_pub_key_str();
                    j_tx["timestamp"]      = recieve_time*1e3; // when it got into mempool
                    j_tx["total_received"] = "0";          // we did not recive any outputs/wtip
...
TxSearch::get_wtip_address_viewkey() const
{
...
{
    WtipTransaction tx_data_existing;

    if (wtip_accounts->tx_exists(acc->id.data, tx_hash, tx_data_existing))
    {
        OMINFO << "\nTransaction " << tx_hash
        ...
        if (wtip_accounts->delete_tx(tx_data_existing.id.data) == 0)
        {
