...
#include "src/YourWorktipsRequests.h"
...
el::Loggers::reconfigureLogger("worktips", defaultConf);

OMINFO << "Worktips is starting";
...
wtipeg::BlockchainSetup bc_setup {nettype, do_not_relay, *config_file_opt};
...
wtipeg::MySqlConnector::url      = config_json["database"]["url"];
wtipeg::MySqlConnector::port     = config_json["database"]["port"];
wtipeg::MySqlConnector::username = config_json["database"]["user"];
wtipeg::MySqlConnector::password = config_json["database"]["password"];
wtipeg::MySqlConnector::dbname   = config_json["database"]["dbname"];
...
auto current_bc_status
        = make_shared<wtipeg::CurrentBlockchainStatus>(
            bc_setup,
            std::make_unique<wtipeg::MicroCore>(),
            std::make_unique<wtipeg::RPCCalls>(bc_setup.deamon_url));
...
wtipeg::ThreadRAII blockchain_monitoring_thread(
            std::thread([current_bc_status]
                        {current_bc_status->monitor_blockchain();}),
            wtipeg::ThreadRAII::DtorAction::join);


OMINFO << "Blockchain monitoring thread started";

// try connecting to the mysql
shared_ptr<wtipeg::MySqlAccounts> mysql_accounts;

try
{
    // MySqlAccounts will try connecting to the mysql database
    mysql_accounts = make_shared<wtipeg::MySqlAccounts>(current_bc_status);
...
wtipeg::MysqlPing mysql_ping {
        mysql_accounts->get_connection(),
        bc_setup.mysql_ping_every_seconds};

wtipeg::ThreadRAII mysql_ping_thread(
...
wtipeg::YourWorktipsRequests open_worktips(mysql_accounts, current_bc_status);
...
// Publish the Worktips Web Wallet API created so that front end can use it
