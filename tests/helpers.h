...
#define TX_FROM_HEX(hex_string)                                              \
    ...
    ASSERT_TRUE(wtipeg::hex_to_tx(hex_string, tx, tx_hash, tx_prefix_hash));  \
    string tx_hash_str = pod_to_hex(tx_hash);                                \
    string tx_prefix_hash_str = pod_to_hex(tx_prefix_hash);                  \
    crypto::public_key tx_pub_key = wtipeg::get_tx_pub_key_from_received_outs(tx);    \
...
#define ACC_FROM_HEX(hex_address)                                            \
         ...
         ASSERT_TRUE(this->wtip_accounts->select(hex_address, acc));
...
#define ADDR_VIEWKEY_FROM_STRING(address_str, viewkey_str, net_type)         \
    ...
    ASSERT_TRUE(wtipeg::addr_and_viewkey_from_string(                         \
