...
{
    wtipeg::BlockchainSetup bc_setup;
    EXPECT_TRUE(true);
}
...
TEST_P(BlockchainSetupTest, ReadInConfigFile)
{
    ...
    string net_name = wtipeg::BlockchainSetup::get_network_name(net_type);

    bool do_not_relay {false};
    string confing_path {"../config/config.json"};

    wtipeg::BlockchainSetup bc_setup {net_type, do_not_relay, confing_path};

    json config_json = wtipeg::BlockchainSetup::read_config(confing_path);
    ...
    wtipeg::BlockchainSetup bc_setup2 {net_type, do_not_relay, config_json};
...
TEST_P(BlockchainSetupTest, ParsingConfigFileFailure)
{
    ...
    json config_json = wtipeg::BlockchainSetup::read_config(config_path);
    ...
    try
    {
        ...
        wtipeg::BlockchainSetup bc_setup {net_type, do_not_relay, wrong_data};
    }
    catch (std::exception const& e)
    {
        EXPECT_TRUE(true);
...
TEST(BlockchainSetupTest, WrongNetworkType)
{
    ...
    {
        // expect throw if cant parase address or viewkey
        wtipeg::BlockchainSetup::get_network_name(net_type);
    }
    catch (std::exception const& e)
    {
        EXPECT_TRUE(true);
    ...
    {
        // expect throw if cant parase address or viewkey
        wtipeg::BlockchainSetup bc_setup {net_type, do_not_relay, config_json};
    }
    catch (std::exception const& e)
    {
        EXPECT_TRUE(true);
...
public:
    ...
    {
        string config_path {"../config/config.json"};
        config_json = wtipeg::BlockchainSetup::read_config(config_path);
    }
...
protected:
    ...
    SetUp()
    {
        bc_setup = wtipeg::BlockchainSetup{net_type, do_not_relay, config_json};
    }  
